...
void insertionSortNome(T *v, int n) {
    for (int i = 1; i < n; i++) {
        ...
        T elemInserir = v[i];
        int j = i - 1;
        while (j >= 0 && elemInserir.nome <
                         v[j].nome) { //se Pessoa não tivesse o operador < implementado, teríamos que adaptar essa parte do código para não utilizar esse operador com o tipo Pessoa (ou seja, fazendo "elemInserir.nome < v[j].nome")
            v[j + 1] = v[j];
            j--;
        }
        v[j + 1] = elemInserir;
...
void insertionSortCpf(T *v, int n) {
    for (int i = 1; i < n; i++) {
        ...
        T elemInserir = v[i];
        int j = i - 1;
        while (j >= 0 && elemInserir.cpf <
                         v[j].cpf) { //se Pessoa não tivesse o operador < implementado, teríamos que adaptar essa parte do código para não utilizar esse operador com o tipo Pessoa (ou seja, fazendo "elemInserir.nome < v[j].nome")
            v[j + 1] = v[j];
            j--;
        }
        v[j + 1] = elemInserir;
...
public:
    bool operator()(const Pessoa &a,
                    const Pessoa &b) const { //retorna true se o local correto do objeto "a" for antes do local correto do objeto "b"
...
int main(int argc, char **argv) {
    Pessoa pessoas[5];
    pessoas[0].nome = "Teste da Silva";
    pessoas[0].cpf = 100;
    pessoas[1].nome = "Jacare da UFV";
    pessoas[1].cpf = 100;
    pessoas[2].nome = "Capivaristo dos Lagos";
    pessoas[2].cpf = 100;
    pessoas[3].nome = "Arthur Bernardes";
    pessoas[3].cpf = 900;
    pessoas[4].nome = "Ubirajara Fernandes Vieira";
    pessoas[4].cpf = 1000;
...
//    insertionSort(pessoas, 5, ComparadorNome());  //comente isto
    ...
    sort(pessoas, pessoas + 5, ComparadorCpfName());
    for (int i = 0; i < 5; i++) cout << pessoas[i].nome << " " << pessoas[i].cpf << "\n";
