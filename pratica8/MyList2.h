...
public:
    ...
    bool compare(iterator t1, iterator t2) const;
    ...
    int size(iterator it = NULL,bool first = true) const {
        if (first) it = dataLast;
        if (it == NULL) return 0;
        if (it->prev == NULL) return 1;
        return 1 + size(it->prev,false);
...
template<class T>
bool MyList2<T>::compare(iterator t1, iterator t2) const {
    //recursive
    if (t1 == NULL || t2 == NULL) return false;
    if (t1 == t2) return false;
    if (t1 == t2->prev) return true;
    return compare(t1, t2->prev);
}
...
int MyList2<T>::eraseMatchingElements(const T &elem) {
    ...
    while (curr != NULL) {
        if (curr->data == elem) {
            ...
            else dataFirst = curr->next;
            if (curr->next != NULL) curr->next->prev = curr->prev;
            else dataLast = curr->prev;
            Node<T> *aux = curr;
            curr = curr->next;
            delete aux;
            dataSize--;
...
template<class T>
const T &
MyList2<T>::deref(const iterator curr) const { //Retorna o elemento da lista apontado por curr (faz uma "derreferencia")
    return curr->data;
}
