...
public:
    ...
    void
    insert(const T &, iterator nodeBefore); //insere o primeiro argumento APOS o nodo apontado pelo segundo elemento
    ...
    iterator
    begin() { return dataFirst; } //Exercicio: e se tivermos uma lista constante, como itera-la para, por exemplo, imprimir os elementos?
    ...
    const iterator
    begin() const { return dataFirst; } //Exercicio: e se tivermos uma lista constante, como itera-la para, por exemplo, imprimir os elementos?
    ...
    void reverse();
    int
    size() const { return dataSize; }; // na STL List, a funcao size() calcula o tamanho da lista dinamicamente (exercicio: qual a ordem de complexidade?)
...
private:
    ...
    void destroy(MyList<T>::iterator curr);
    void destroy();
};
...
template<class T>
void MyList<T>::destroy(MyList<T>::iterator curr) {
    if (curr == NULL) return;
//    destroy(curr->next);
//    delete curr;
    while (curr != NULL) {
...
MyList<T>::MyList(int n, const T &init) {
    ...
    for (int i = 0; i < n; i++)
        push_back(
                init); //implementacao pode ser mais eficiente se evitar atualizar o apontador "dataLast" em cada iteracao
}
...
template<class T>
const T &
MyList<T>::deref(const iterator curr) const { //Retorna o elemento da lista apontado por curr (faz uma "derreferencia")
    return curr->data;
}
