...
void reverse(MyList2<T> &lista) {
    if(lista.empty() || lista.size() == 1)
        return;
    MyList2<int>::iterator init = lista.begin();
    auto lastIterator = lista.begin();
    auto secondLastIterator = ++lista.begin();
    int size=0;
    while (secondLastIterator != lista.end()) {
        lastIterator++;
        secondLastIterator++;
        size++;
    }
    size++;
    int newSize = size/2;
    while (init != lastIterator) {
        swap(*init, *lastIterator);
        size--;
        if(size == newSize)
            break;
        init++;
